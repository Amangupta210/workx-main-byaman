  ...
  return (
    ...
    >
      <div
        ...
        className="flex max-h-[85vh] w-full max-w-lg flex-col rounded-2xl bg-card text-card-foreground shadow-xl ring-1 ring-border"
      >
        <header className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs text-muted-foreground hover:bg-secondary disabled:opacity-50"
              title="Try to drain the queue now"
            >
              {draining ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />}
              Drain
            </button>
            <button onClick={onClose} aria-label="Close" className="rounded-md p-1.5 hover:bg-secondary">
              <X size={14} />
            </button>
          </div>
        </header>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        {j.status === 'failed' && <AlertTriangle size={11} className="text-destructive" />}
        ...
        {failed.length > 0 && (
          <footer className="flex items-center justify-between border-t border-border px-4 py-2 text-xs">
            ...
            <div className="flex items-center gap-2">
              <button onClick={retryAll} className="rounded-md border border-input px-2 py-1 hover:bg-accent">
                Retry all failed
              </button>
              <button onClick={clearFailed} className="rounded-md px-2 py-1 text-destructive hover:bg-destructive/10">
                Clear failed
              </button>
            </div>
          </footer>
