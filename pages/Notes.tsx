            ...
            <div className="relative" ref={exportRef}>
              ...
              {exportOpen && (
                <div role="menu" className="absolute right-0 z-30 mt-1 w-56 overflow-hidden rounded-md border border-border bg-popover text-popover-foreground shadow-lg">
                  <button
                    role="menuitem"
                    onClick={() => { try { exportNotesToPdf(filtered.length ? filtered : notes); setExportOpen(false); toast({ title: 'PDF exported' }); } catch (e) { toast({ title: 'Export failed', description: String(e), variant: 'destructive' }); } }}
                  ...
                  <button
                    role="menuitem"
                    onClick={() => { try { exportNotesToPdf(notes, 'sticky-notes-all.pdf'); setExportOpen(false); toast({ title: 'PDF exported' }); } catch (e) { toast({ title: 'Export failed', description: String(e), variant: 'destructive' }); } }}
                    className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-accent"
                  >
                  ...
                  <button
                    role="menuitem"
                    onClick={() => { try { exportNotesToTxt(filtered.length ? filtered : notes); setExportOpen(false); toast({ title: 'TXT exported' }); } catch (e) { toast({ title: 'Export failed', description: String(e), variant: 'destructive' }); } }}
                    className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-accent"
                  >
                  ...
                  <button
                    role="menuitem"
                    onClick={() => { try { exportNotesToTxt(notes, 'sticky-notes-all.txt'); setExportOpen(false); toast({ title: 'TXT exported' }); } catch (e) { toast({ title: 'Export failed', description: String(e), variant: 'destructive' }); } }}
                    className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-accent"
                  >
